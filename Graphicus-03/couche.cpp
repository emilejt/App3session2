...
bool Couche::changerEtat(string etatDemande)
{
	...
	else if (etatDemande.compare("cachee") == 0)
	{
		etat = 4;
	}
	else
...
void Couche::afficherCouche(ostream&)
{
	cout << vect;
}
...
const Forme* Couche::operator[](int index) const
{
	return vect[index];
}

Forme* Couche::operator[](int index)
{
	return vect[index];
}
